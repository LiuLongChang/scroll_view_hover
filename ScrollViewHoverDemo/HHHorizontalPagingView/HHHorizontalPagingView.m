...
@property(nonatomic,retain)NSLayoutConstraint* indicatorViewCenterConstraint;
        ...
        for(int i = 0; i < [self.segmentButtons count]; i++) {
            ...
            if(i == 0) {
                ...
                if (self.indicatorView == nil) {
                    ...
                    NSLayoutConstraint * topConstraintU = [NSLayoutConstraint constraintWithItem:self.indicatorView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:_segmentView attribute:NSLayoutAttributeBottom multiplier:1 constant:0];
                    NSLayoutConstraint * heightConstraintU = [NSLayoutConstraint constraintWithItem:self.indicatorView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:0 multiplier:1 constant:2];
                    NSLayoutConstraint * widthConstraintU = [NSLayoutConstraint constraintWithItem:self.indicatorView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:_segmentView attribute:NSLayoutAttributeWidth multiplier:0.5 constant:0];
                    NSLayoutConstraint * centerConstraintU = [NSLayoutConstraint constraintWithItem:self.indicatorView attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:segmentButton attribute:NSLayoutAttributeCenterX multiplier:1 constant:0];
                    self.indicatorViewCenterConstraint = centerConstraintU;

                    [_segmentView addConstraint:topConstraintU];
                    [_segmentView addConstraint:heightConstraintU];
                    [_segmentView addConstraint:widthConstraintU];
                    [_segmentView addConstraint:centerConstraintU];
                }
    ...
    for(UIButton *b in self.segmentButtons) {
        if(b.tag - pagingButtonTag == buttonPage) {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                self.indicatorViewCenterConstraint.constant = (buttonPage)*([[UIScreen mainScreen] bounds].size.width/self.segmentButtons.count);
...
-(NSMutableArray *)segmentButtonConstraintArray{
    if (!_segmentButtonConstraintArray) {_segmentButtonConstraintArray = [NSMutableArray array];}
    return _segmentButtonConstraintArray;
}

- (NSMutableArray<UIScrollView *> *)contentViewArray{
    if (!_contentViewArray) {_contentViewArray = [[NSMutableArray alloc] init];};   return _contentViewArray;
}
